...
def load_img_name_list(dataset_path):
    img_name_list = np.loadtxt(dataset_path,str)
...
def get_label_path(root_dir, img_name, is_tif=False):
...
class CDDataset(ImageDataset):
    ...
    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,image_type='jpg',
                to_tensor=True):
    ...
    def __getitem__(self, index):
        ...
        if label.ndim == 3:     # 针对DSIFN数据集下训练集和测试集的shape不一致问题
            label = label[:, :, 0]

        #  while max of label is 255, we need to normalize it to 0-1
