...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        # 针对新加的模块设置大学习率
        # base_paramer = list(self.net_G.encoder.resnet_stream.parameters())+  list(self.net_G.encoder.resnet.layer1.parameters()) + \
        #                 list(self.net_G.encoder.resnet.layer2.parameters()) + list(self.net_G.encoder.resnet.layer3.parameters()) + \
        #                 list(self.net_G.encoder.resnet.layer4.parameters())
        # base_parmer_ids = [id(p) for p in base_paramer]
        # # new add module paramers
        # new_paramer = filter(lambda p:id(p) not in base_parmer_ids,self.net_G.parameters())
        ...
        if args.optimizer == "sgd":
            ...
            self.optimizer_G = optim.SGD(self.net_G.parameters(), lr=self.lr,
                                     momentum=0.9,
                                     weight_decay=5e-4)# origion 5e-4
            # 分组学习率设置
            # self.optimizer_G = optim.SGD([{
            #     "params": base_paramer,"lr":0.001,
            #     "params": new_paramer,
            #     }], 
            ...
            self.optimizer_G = optim.AdamW(self.net_G.parameters(), lr=self.lr,
                                    betas=(0.9, 0.999), weight_decay=0.01)# 0.01 default
        ...
        self.G_loss = None
        self.sfd_loss = None
        self.Semantic_feature_logit = None
        self.is_training = False
        self.batch_id = 0
        self.epoch_id = 0
        self.checkpoint_dir = args.checkpoint_dir
        self.vis_dir = args.vis_dir
        self.sfd_weight = args.sfd_loss_weight
        ...
        # self.weights = tuple(args.multi_pred_weights) # 多尺度推理时的权重
        print(args.loss)
        if args.loss == 'ce':
    ...
    def _visualize_pred(self):
        pred = torch.argmax(self.G_final_pred, dim=1, keepdim=True)  # 找到最大概率的索引(返回 0/1)
        pred_vis = pred * 255
        return pred_vis
    ...
    def _scale_imags(self, imgs, scales):
        B, C, H, W = imgs.shape
        img_scale = torch.zeros(B,C,int(H*scales),int(W*scales)).to(self.device)
        for step,img in enumerate(imgs):
            img = img.unsqueeze(0).float()
            img = F.interpolate(img, scale_factor=scales, mode="bilinear")
            img_scale[step] = img
        return img_scale
    
    # def _saved_multi_scale_gt(self,imgs,scale):     # 保存多尺度gt
    #     for step,img in enumerate(imgs):
    #         img = img.cpu().numpy().transpose(1,2,0) # (H,W,C)
    #         img = np.uint8(img*255)
    #         cv2.imwrite('scale_'+str(scale)+'_'+str(step)+'.jpg',img)
    #     print('saved')
        
    # def _create_multi_scale_gt(self):
    #     target = self.batch['L'].to(self.device).detach() # (B, 1, H, W)
    #     B, C, H, W = target.shape
    #     self.multi_scale_gt = []
    #     scales = [0.25,0.5,1]
    #     img_scale_raw = target
        
    #     img_scale_64 = self._scale_imags(img_scale_raw, scales[0])
    #     img_scale_128 = self._scale_imags(img_scale_raw, scales[1])
    #     # 查看保存是否正确
    #     # self._saved_multi_scale_gt(img_scale_64,64)
    #     # self._saved_multi_scale_gt(img_scale_128,128)
    #     # self._saved_multi_scale_gt(img_scale_raw,256)
        
    #     return img_scale_64.to(self.device), img_scale_128.to(self.device), img_scale_raw
    ...
    def _update_metric(self):
        ...
        # target_64, target_128, target = self._create_multi_scale_gt()
    ...
    def _collect_running_batch_states(self):
        ...
        # TODO: visualize the prediction
        # if np.mod(self.batch_id, 500) == 1 and self.batch_id != 1:
    ...
    def _forward_pass(self, batch):
        ...
        if self.is_training:    
            self.G_pred,self.sfd_loss = self.net_G(img_in1, img_in2)
        else:
            self.G_pred = self.net_G(img_in1, img_in2)
        # if self.multi_scale_infer == "True": # 多尺度推理
        #     self.G_final_pred = torch.zeros(self.G_pred[0].size()).to(self.device)
        #     for pred in self.G_pred:
        #         if pred.size(2) != self.G_pred[0].size(2): # 当尺寸不一致的时候使用最近邻插值至一样大小
        #             self.G_final_pred = self.G_final_pred + F.interpolate(pred, size=self.G_pred[0].size(2), mode="nearest")
        #         else:
        #             self.G_final_pred = self.G_final_pred + pred
        #     self.G_final_pred = self.G_final_pred/len(self.G_pred) # 取平均
        # else:
        if self.is_training:
            self.G_final_pred = self.G_pred # 2,256,256
        else:
    ...
    def _backward_G(self):
        ...
        self.G_loss = self._pxl_loss(self.G_pred, gt)+self.sfd_weight*self.sfd_loss
