...
def cd_loss_DSIFN(input,target):
    '''
...
def cross_entropy(input, target, weight=None, reduction='mean',ignore_index=255):
    ...
    :param input: torch.Tensor, N*2*H*W
    ...
    if input.shape[-1] != target.shape[-1]: # 确保输入和标签的尺寸一致
...
def binary_cross_entropy(input, target, weight=None, reduction='mean'):
    ...
    return F.binary_cross_entropy_with_logits(input=input, target=target, weight=weight,
                           reduction=reduction)
